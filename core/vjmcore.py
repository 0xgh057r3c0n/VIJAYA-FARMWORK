...
def check_sudo():
    if os.geteuid() != 0:
        print_in_red("Must be run as sudo!")
        sys.exit(1)

prefix = os.getenv("PREFIX")
if not prefix:
    user_home = os.path.expanduser("~")
    prefix = os.path.join(user_home, "Vijaya")

configBase = os.path.expanduser("~")

script_dir = os.path.dirname(os.path.abspath(__file__))
configFile = os.path.join(script_dir, "../vijaya.conf")

cache_1 = os.path.join(prefix, "tmp", "vijaya_1")
os.makedirs(os.path.dirname(cache_1), exist_ok=True)
check_sudo()

print(f"Prefix is set to: {prefix}")
print(f"Config Base (HOME) is set to: {configBase}")
print(f"Config File is set to: {configFile}")
print(f"Cache Path (vijaya_1) is set to: {cache_1}")

def repo_check(sources_list):
    if os.path.isfile(os.path.join(prefix, "etc", "apt", "sources.list.d", sources_list)):
        return True
    return False
...
def backtomenu_option():
    if not readStatus():
        lolcat_print(backtomenu_banner)
        backtomenu = input("vjya > ")
